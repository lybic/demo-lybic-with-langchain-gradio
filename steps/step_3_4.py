...
async def playground(user_input, history=None):
    ...
    try:
        async with LybicClient() as client:
            ...
            print(f"debug:parse the model output: {response} ")
            action_dto = await computer_use.parse_model_output(
                dto.ComputerUseParseRequestDto(model="seed",
                textContent=str(response),
            ))
            print("debug:execute the action")
            await computer_use.execute_computer_use_action(sandbox_id,dto.ComputerUseActionDto(action=action_dto.actions[0]))

            memory.chat_memory.add_message(message[0])
            memory.chat_memory.add_ai_message(response)

            history.append([user_message_for_history, response])

            print("debug:show the screenshot")
            _, img, _ = await sandbox.get_screenshot(sandbox_id)
            img.show()
